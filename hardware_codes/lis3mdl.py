...
"""
## original in this library, but according to datasheet seems to be incorrect (check on Nov. 7, 2024)
...
"""
## add on Nov. 7, 2024 ##
RATE_1000_HZ = const(0b000001)
RATE_560_HZ = const(0b010001)
RATE_300_HZ = const(0b100001)
RATE_155_HZ = const(0b110001)
## add on Nov. 7, 2024 ##
...
class LIS3MDL:
    ...
    **Quickstart: Importing and using the device**
    ...
    _z_operating_mode = CBits(2, _CTRL_REG4, 2) ## add on Nov. 13, 2024
    
    
    
    _raw_magnetic_data = RegisterStruct(_DATA, "<hhh")
    
    _raw_offset_data = RegisterStruct(_OFFSETS, "<hhh") ## add on Nov. 14, 2024
    ...
    def __init__(self, i2c, address: int = 0x1C) -> None:
        ...
        self._z_operating_mode = UHP ## add on Nov. 13, 2024
        #print(self._z_operating_mode)
    ...
    def data_rate(self) -> str:
        ...
        +-----------------------------------+----------------------+
        | :py:const:`lis3mdl.RATE_155_HZ`   | :py:const:`0b000001` | ## wrong
        +-----------------------------------+----------------------+
        | :py:const:`lis3mdl.RATE_300_HZ`   | :py:const:`0b010001` | ## wrong
        +-----------------------------------+----------------------+
        | :py:const:`lis3mdl.RATE_560_HZ`   | :py:const:`0b100001` | ## wrong
        +-----------------------------------+----------------------+ 
        | :py:const:`lis3mdl.RATE_1000_HZ`  | :py:const:`0b110001` | ## wrong
        +-----------------------------------+----------------------+
        | :py:const:`lis3mdl.RATE_1000_HZ`  | :py:const:`0b000001` | 
        +-----------------------------------+----------------------+
        | :py:const:`lis3mdl.RATE_560_HZ`   | :py:const:`0b010001` |
        +-----------------------------------+----------------------+
        | :py:const:`lis3mdl.RATE_300_HZ`   | :py:const:`0b100001` | 
        +-----------------------------------+----------------------+ 
        | :py:const:`lis3mdl.RATE_155_HZ`   | :py:const:`0b110001` | 
        +-----------------------------------+----------------------+ 
        """
        ...
        return values[self._data_rate] ## treated _data_rate directly as index, which is incorrect
    ...
    def data_rate(self, value: int) -> None:
        ...
        #print("value in lis3mdl.py: ", value)
        self._data_rate = value
        #print("_data_rate in lis3mdl.py: ", self._data_rate)
    ...
    ## add on Nov. 14 2024 ##
    @property
    def offset_magnetic_raw(self) -> Tuple[float, float, float]:
        """
        Magnetometer values in microteslas
        """
        rawx, rawy, rawz = self._raw_magnetic_data
        offset_x, offset_y, offset_z = self._raw_offset_data
        x = (rawx - offset_x) 
        y = (rawy - offset_y)
        z = (rawz - offset_z)
        #print("offset raw: ", offset_x, " ", offset_y, " ", offset_z)

        return x, y, z
    
    
    ## add on Nov. 7 2024 ##
    def read_ctrl_reg1(self) -> int:
        """
        Reads a byte from CTRL_REG1 and returns it.
        """
        return self._i2c.readfrom_mem(self._address, _CTRL_REG1, 8)[0]  # Read 1 byte from CTRL_REG1
    
    ## add on Nov. 8 2024 ##
    def read_ctrl_reg2(self) -> int:
        """
        Reads a byte from CTRL_REG2 and returns it.
        """
        return self._i2c.readfrom_mem(self._address, _CTRL_REG2, 8)[0]  # Read 1 byte from CTRL_REG2
    
    ## add on Nov. 13 2024 ##
    def read_ctrl_reg3(self) -> int:
        """
        Reads a byte from CTRL_REG3 and returns it.
        """
        return self._i2c.readfrom_mem(self._address, _CTRL_REG3, 8)[0]  # Read 1 byte from CTRL_REG3
    
    ## add on Nov. 13 2024 ##
    def read_ctrl_reg4(self) -> int:
        """
        Reads a byte from CTRL_REG4 and returns it.
        """
        return self._i2c.readfrom_mem(self._address, _CTRL_REG4, 8)[0]  # Read 1 byte from CTRL_REG4
