...
This script directly uses simple regression model to perform localization of 2 magnets, retrieving position values in x, y, z directions
"""
...
async def run_queue_consumer(queue: asyncio.Queue):
    model = load_model('z1_44_z2_44_regression_model_lr0.001_epochs200_batch16_folds15.h5', custom_objects={'mse': MeanSquaredError()})
    #model = load_model('z1_44_z2_44_3ori_regression_model_lr0.001_epochs50_batch16_folds5.h5', custom_objects={'mse': MeanSquaredError()})
    scaler = joblib.load('scaler_regression_z1_44_z2_44_1ori.joblib')
    
    #model = load_model('2mag_sameH_44546474_1ori_regression_model_lr0.001_epochs200_batch16_folds5.h5', custom_objects={'mse': MeanSquaredError()})
    #scaler = joblib.load('scaler_regression_2mag_sameH_44546474_1ori.joblib')
...
###Initialization for rolling median
    ...
    while True:
        ...
        print(len(result[0]))
        result = result[0]*0.001 # [1 2 3]
        #print(result)

        for i in range(len(result)//3):
            print(f"x{i}: {result[i*3]*1000}, y{i}: {result[i*3+1]*1000}, z{i}: {result[i*3+2]*1000}") # already in mm 

        #plot_cv2grey(data)
        plot_cv2localization_30_multi_magnets_regression(result,grid_canvas) 
